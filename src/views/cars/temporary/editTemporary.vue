...
export default {
  ...
  created() {
    this.temporary_id = this.$route.query.temporary_id;
    this.getInfo();
  },
  methods: {
    goBack() {
    ...
    getInfo() {
      ...
      this.$axios.post(url.temporaryInfo, data).then(res => {
    ...
    submit() {
      let obj = {
        ...
        this.$axios
          .post(url.editTemporary, data)
          .then(res => {
            if (res.data.status == 1) {
              this.$message.success(res.data.msg);
              this.$router.push("/getTemporary");
            }
          })
          .catch(req => {
