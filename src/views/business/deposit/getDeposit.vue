<template>
  <div style="height:100%">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>定金列表</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="card">
      <p class="title">
        <span>定金列表</span>
        <el-button size="mini" @click="addOrder">添加定金</el-button>
      </p>
      <!-- 列表 -->
      <el-table
        :data="tableData"
      ...
      >
        <el-table-column label="">
          ...
          <el-table-column prop="id" label="定金ID" width="80px"></el-table-column>
          <el-table-column prop="amount" label="金额" width="250px"></el-table-column>
          ...
          <el-table-column prop="type" label="类型" width="130px">
              <template slot-scope="scope">
                  {{scope.row.type==1?'销售订单':'采购订单'}}
              </template>
          </el-table-column>
          <el-table-column prop="type_name" label="销售/采购" width="160px"></el-table-column>
        </el-table-column>
...
export default {
  ...
  methods: {
    async getList() {
      ...
      await this.$axios.post(url.deposit, data).then(res => {
    ...
    delBusiness(row) {
      ...
      this.$confirm("确定删除吗？", "提示", {
        ...
        .then(() => {
          this.$axios.post(url.delDeposit, data).then(res => {
            if (res.data.status == 1) {
