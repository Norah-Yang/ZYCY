<template>
  <div style="height:100%">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>费用分类管理</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="card">
      <p class="title">
        <span>费用分类明细</span>
        <el-button size="mini" @click="addOrder">添加费用分类</el-button>
      </p>
      <!-- 列表 -->
      <el-table
        :data="tableData"
...
export default {
  ...
  methods: {
    async getList() {
      ...
      await this.$axios.post(url.cost,data).then(res => {
    ...
    delBusiness(row) {
      ...
      this.$confirm("确定删除吗？", "提示", {
        ...
        .then(() => {
          this.$axios.post(url.delCost, data).then(res => {
            if (res.data.status == 1) {
