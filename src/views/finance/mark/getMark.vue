<template>
  <div style="height:100%">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>销售费用管理</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="card">
      <p class="title">
        <span>销售费用管理</span>
        <el-button size="mini" @click="addOrder">添加销售费用</el-button>
      </p>
      <!-- 列表 -->
      <el-table
        :data="tableData"
        ...
        <el-table-column label="">
          ...
          <el-table-column prop="type" label="费用类型">
              <template slot-scope="scope">
                  <span :class="scope.row.type==1?'red':'green'">{{scope.row.type==1?'销售':'采购'}}</span>
          ...
          <el-table-column prop="title" label="费用名称"></el-table-column>
        </el-table-column>
...
export default {
  ...
  methods: {
    async getList() {
      ...
      await this.$axios.post(url.mark,data).then(res => {
    ...
    delBusiness(row) {
      ...
      this.$confirm("确定删除吗？", "提示", {
        ...
        .then(() => {
          this.$axios.post(url.delMark, data).then(res => {
              console.log(res)
            if (res.data.status == 1) {
